...
export default function PDFEditor({ hideChatFill = false, hideAutoFill = false }: { hideChatFill?: boolean; hideAutoFill?: boolean } = {}) {
  ...
  const applyAutoFill = useCallback((filled: FilledField[]) => {
    ...
    filled.forEach(({ name, value }) => {
      ...
      // textLayer: place text just above the form line.
      //   textRef = ty (underscore baseline in PDF y-from-bottom coords)
      //   pdfY  = pageH - ty - ascender - 1  →  text baseline lands ~1pt above the underline.
